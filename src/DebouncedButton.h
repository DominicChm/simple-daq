...
private:
    ...
    bool debounced{};
public:
    explicit DebouncedButton(uint8_t buttonPin, uint32_t debounceMs = 1000, bool inverted = false,
    ...
    bool isTriggered() { //TODO - Consider using an interrupt? Would make the lib less portable...
        debounced &=
                digitalRead(buttonPin) == triggeredOn; //Prevent multiple triggers without at least one state change.
